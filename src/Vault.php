...
class Vault {
	...
	static public function fileClassData() {
		...
		if (!array_key_exists(self::$namespace, $prefix)) _::error('PSR4 ' . self::$namespace . ' isn\'t registred', FATAL_ERROR);
		$dirs = $prefix[self::$namespace];
	...
	private function build($content = [], $key = '', $cipher = '') {
		static $lf = "\n", $t = "\t";
		...
		$content = '<?php' . $lf . $lf;
		$content .= 'namespace ' . __NAMESPACE__ . ';' . $lf . $lf;
		...
		$content .= $this->trVal($content, $t, ';') . $lf;
		$content .= '}' . $lf;
